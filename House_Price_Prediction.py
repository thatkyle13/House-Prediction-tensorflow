...
#
#       This is a Very simple prediction based on house size, implemented
#
...
# generation of house prices from house size with a random noise added
np.random.seed(42)
...
#  Define the variable holding the size_factor and price we set during training.
#  We initialize them to some random values based on the normal distribution.
tf_size_factor = tf.Variable(np.random.randn(), name="size_factor")
tf_price_offset = tf.Variable(np.random.randn(), name="price_offset")

#  2. Defice the operations for the precting values - predicted price = (size_factor * house_size) + price_offset
#  Notice, the use of tensorflow add and multiply functions. These add the operations to the computation graph,
...
with tf.Session() as sess:
    ...
    # keep iteration the training data
    ...
    #Plot of taining and test data, and learned regression
    ...
    plt.plot(test_house_size, test_house_price, 'mo', label = 'Training data')
